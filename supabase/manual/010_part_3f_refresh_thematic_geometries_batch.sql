...
-- Parte 3F: refresh em lote das geometrias tematicas.
-- Rode depois da parte 3E. Repita ate processed_count = 0.
...
prepared as (
  ...
  from public.geo_thematic_layers target
  join batch on batch.id = target.id
),
updated as (
  update public.geo_thematic_layers target
...
select
  'geo_thematic_layers_geom' as step,
